...
char fastpath[100];
   ...
   openlog("grdnsd", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
   ...
   if ((fastdns_port = getenv("GRDNS_PORT"))) fastdns_port_int = atoi(fastdns_port);
   else {
        syslog (LOG_NOTICE,"$GRDNS_PORT environment not set.\n");
        syslog (LOG_NOTICE,"Aborting\n");
        exit(EXIT_FAILURE);
   }
...
void doprocessing (int sock, /*@unused@*/ char* clip) {
   ...
   /***********************************************************************************/
        ...
        snprintf(outline, (size_t) 500,"%s/%d/%d/r%d",fastpath,ip_octet[0],ip_octet[1],ip_octet[2]);
	dns = fopen(outline,"r");
